...
function operator(pro) {
  ...
  pro.forEach((e) => {
    ...
    // ======【新增：仅提取商家名（第一段），其它逻辑不动】======
    // 例如：DMPRO-US-direct => DMPRO
    const __vendor = ens && ens.includes("-") ? ens.split("-")[0].trim() : "";
    if (__vendor) e.__vendor = __vendor;
    // ======【新增结束】======
    ...
    // ======【方案 C：地区匹配改为大小写不敏感（仅此处修改）】======
    const nameLower = String(e.name || "").toLowerCase();
    const findKey = AMK.find(([k]) => nameLower.includes(String(k).toLowerCase()));
    // ======【方案 C 修改结束】======
  ...
  // ======【新增：最后追加商家名（不影响原编号/过滤/排序）】======
  pro.forEach((e) => {
    if (e.__vendor) {
      e.name = e.name + FGF + e.__vendor;
      delete e.__vendor;
    }
  });
  // ======【新增结束】======
